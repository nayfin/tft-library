...
import { FormConfig, ControlType } from './dynamic-field-config';
@Component({
...
export class DynamicFormComponent implements OnInit {
  ...
  buildFormGroupFromConfig(config: FormConfig) {
    ...
    config.fields.forEach( (controlConfig: any) => {
      ...
      if ( controlConfig.controlType === ControlType.GROUP ) {
      ...
      } else if (controlConfig.controlType === ControlType.ARRAY) {
        group.addControl(controlConfig.controlName, this.fb.array([]));
      } else {
